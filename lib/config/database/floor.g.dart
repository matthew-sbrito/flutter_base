    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Movement` (`id` INTEGER NOT NULL, `quantity` INTEGER NOT NULL, PRIMARY KEY (`id`))');
...
class _$MovementDao extends MovementDao {
  _$MovementDao(this.database, this.changeListener)
      : _queryAdapter = QueryAdapter(database, changeListener),
        _movementInsertionAdapter = InsertionAdapter(
            database,
            'Movement',
            (Movement item) =>
                <String, Object?>{'id': item.id, 'quantity': item.quantity},
            changeListener);
  ...
  Future<List<Movement>> findAll() async {
    return _queryAdapter.queryList('SELECT * FROM Movement',
        mapper: (Map<String, Object?> row) =>
            Movement(row['id'] as int, row['quantity'] as int));
  }
